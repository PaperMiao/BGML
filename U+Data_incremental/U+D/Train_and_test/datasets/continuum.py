...
def continuum(root='/datasets/', name='reddit', data_type='train', task_type = 0, k_hop = 1, download=True, thres_nodes = 20, first_shard = None, second_shard = None, Unlearning = False):
    name = name.lower()
    if name in ['reddit', 'flickr']:
        ...
        return ContinuumShard(root=root, name=name, data_type=data_type, task_type = task_type, download=download,k_hop = k_hop, thres_nodes = thres_nodes, first_shard = first_shard, second_shard = second_shard, Unlearning = Unlearning)
    else:
