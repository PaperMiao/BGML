...
class DataStore:
    ...
    def save_train_test_split(self, train_indices, test_indices, data_incremental_indicies):
        self.logger.info('saving train test split data')
        pickle.dump((train_indices, test_indices, data_incremental_indicies), open(self.train_test_split_file, 'wb'))
    ...
    def save_second_community_data(self, community_to_node_all, suffix='second'):
        self.logger.info('saving second community data')
        pickle.dump(community_to_node_all, open(self.community_file + suffix, 'wb'))

    def load_second_community_data(self, suffix='second'):
        self.logger.info('loading second community data from: %s'%(self.community_file + suffix))
        return pickle.load(open(self.community_file + suffix, 'rb'))
    ...
    def save_second_shard_data(self, second_shard_data, suffix):
        self.logger.info('saving second shard data: %s'%(suffix))
        pickle.dump(second_shard_data, open(self.shard_file + 'second' + suffix, 'wb'))

    def load_second_shard_data(self, suffix):
        self.logger.info('loading shard data: %s'%(suffix))
        return pickle.load(open(self.shard_file + 'second' + suffix, 'rb'))
