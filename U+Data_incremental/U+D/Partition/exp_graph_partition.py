...
class ExpGraphPartition(Exp):
    ...
    def train_test_split(self): # 训练、测试划分
        if self.args['is_split']:
            ...
            all_node_indicies = np.arange((self.data.num_nodes))
            self.train_indices, self.test_indices = train_test_split(all_node_indicies, test_size=self.args['test_ratio'], train_size=self.args['train_ratio'], random_state=100)
            self.data_incremental_indicies = list(set(all_node_indicies)-set(self.train_indices)-set(self.test_indices))
            ## print(len(self.train_indices), len(self.test_indices), len(self.data_incremental_indicies))
            ## 这个划分函数是包里自带的
            ## 控制在应用拆分之前应用于数据的混洗 random_state , 根据随机参数打乱
            ## 测试集比例是自己定的，Coauthor类数据集效果会不会和这个有关？好像不是。。。。之前的训练、测试、验证的比例为0.1 0.8 0.1
            self.data_store.save_train_test_split(self.train_indices, self.test_indices, self.data_incremental_indicies) ## 将划分的索引存下来
            ...
            self.data.incremental_mask = torch.from_numpy(np.isin(np.arange(self.data.num_nodes), self.data_incremental_indicies))
        else:
            self.train_indices, self.test_indices, self.data_incremental_indicies = self.data_store.load_train_test_split() ## 因为数据集划分是随机的，所以可以按照之前划分的数据集来
    ...
    def gen_train_graph(self):
        ...
        incremental_edge_indices = np.logical_or(np.isin(edge_index[0], self.data_incremental_indicies),
                                          np.isin(edge_index[1], self.data_incremental_indicies))
        test_and_incremental_edge_indices = np.logical_or(test_edge_indices, incremental_edge_indices)
        train_edge_indices = np.logical_not(test_and_incremental_edge_indices)
