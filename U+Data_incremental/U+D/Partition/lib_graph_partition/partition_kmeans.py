    ...
    def partition(self):
        ...
        if not self.args['is_constrained']:
            ...
            if self.partition_method == 'sage_km': ## 和base的区别在于，用的不是树结构，采用的排序的方法
                ...
                community, km_deltas, centroid = cluster.clustering()
                pickle.dump(km_deltas, open(config.ANALYSIS_PATH + "partition/bkm_" + self.args['dataset_name'], 'wb'))
                pickle.dump(centroid, open(config.ANALYSIS_PATH + "partition/first_centroid_bkm_" + self.args['dataset_name'], 'wb'))
                ## pickle.dump()函数将Python对象序列化为字节流，并将字节流写入文件中。
        ...
        for i in range(self.num_shards):
            ...
            community_2, km_deltas_2, centroid_2 = cluster_2.clustering() ## 开始聚类
            pickle.dump(km_deltas_2, open(config.ANALYSIS_PATH + "partition/bkm_2_" + self.args['dataset_name'] + str(i), 'wb'))
            pickle.dump(centroid_2, open(config.ANALYSIS_PATH + "partition/second_centroid_bkm2_" + self.args['dataset_name'] + str(i), 'wb'))
                ## pickle.dump()函数将Python对象序列化为字节流，并将字节流写入文件中。
