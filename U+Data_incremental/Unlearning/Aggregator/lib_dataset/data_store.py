...
class DataStore:
    ...
    def save_first_shard_data(self, shard_data):
        self.logger.info('saving shard data')
        pickle.dump(shard_data, open(self.shard_file, 'wb'))

    def load_first_shard_data(self):
        self.logger.info('loading first shard data')
    ...
    def load_second_shard_data(self, suffix):
        self.logger.info('loading second shard data: %s'%(suffix))
    ...
    def save_posteriors(self, posteriors, suffix=''):
        torch.save(posteriors, self.posteriors_file + suffix)

    def load_posteriors(self):
        return torch.load(self.posteriors_file)
